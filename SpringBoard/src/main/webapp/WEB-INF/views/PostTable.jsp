...
<html lang="en">
<head>
<link href="/SpringBoard/resources/board.css?after" rel="stylesheet"
	type="text/css" />
</head>
<body>
	<div id="navigation">
	...
	<table>
		...
		<c:forEach begin="${first}" end="${end}" varStatus="status">
			...
			<tr>
				...
				<td class=title><a
					href=PostView?id=${boardItemList[i+first].id}&boardId=${boardItemList[i+first].boardId}
					id=movePage>${boardItemList[i+first].title}</a></td>
				<td class=date>${boardItemList[i+first].date}</td>
			<tr>
		</c:forEach>
	</table>
	<div>
